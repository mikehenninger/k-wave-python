...
def scale_source_terms_func(
    ...
    if not check_conditions(
        flags.nonuniform_grid, flags.source_uy, flags.source_uz, flags.transducer_source
    ):
        return
    ...
    apply_pressure_source_correction(
        flags.source_p, flags.use_w_source_correction_p, source, dt
    )

    scale_pressure_source(
        flags.source_p,
        source,
        kgrid,
        N,
    ...
    apply_velocity_source_corrections(
        flags.use_w_source_correction_u,
        flags.source_ux,
        flags.source_uy,
        flags.source_uz,
        source,
    ...
    # TRANSDUCER SOURCE
    # =========================================================================
    transducer_input_signal = scale_transducer_source(
        flags.transducer_source, transducer_input_signal, c0, dt, dx, u_source_pos_index
    )
    return transducer_input_signal


def check_conditions(
    is_nonuniform_grid, is_source_uy, is_source_uz, is_transducer_source
):
    """
    ...
    if is_nonuniform_grid and (is_source_uy or is_source_uz or is_transducer_source):
        logging.log(
            logging.WARN,
            "source scaling not implemented for non-uniform grids with given source condition",
        )
        return False
    return True


def apply_pressure_source_correction(
    is_source_p, use_w_source_correction_p, source, dt
):
    """
...
def scale_pressure_source(
    is_source_p, source, kgrid, N, c0, dx, dt, p_source_pos_index, is_nonuniform_grid
):
    """
    ...
    if source.p_mode == "dirichlet":
        ...
        if is_nonuniform_grid:
            source.p = scale_pressure_source_nonuniform_grid(
                source.p, kgrid, c0, N, dt, p_source_pos_index
            )

        else:
            source.p = scale_pressure_source_uniform_grid(
                source.p, c0, N, dx, dt, p_source_pos_index
            )
...
def scale_pressure_source_dirichlet(source_p, c0, N, p_source_pos_index):
    if c0.size == 1:
        ...
        scale = 1.0 / (
            N * np.expand_dims(c0.ravel(order="F")[mask.ravel(order="F")] ** 2, axis=-1)
        )
        source_p[ind, :] *= scale
...
def scale_pressure_source_nonuniform_grid(
    source_p, kgrid, c0, N, dt, p_source_pos_index
):
    x = kgrid.x
    xn = kgrid.xn
    yn = kgrid.yn
    ...
    if kgrid.dim == 1:
        ...
        grid_point_sep[1:-1, 1:-1] = (
            x_size * (xn[2:, 1:-1] - xn[0:-2, 1:-1]) / 4
...
def scale_pressure_source_uniform_grid(source_p, c0, N, dx, dt, p_source_pos_index):
    if c0.size == 1:
        ...
        mask = p_source_pos_index.flatten("F")[ind]
        scale = (2.0 * dt) / (
            N * np.expand_dims(c0.ravel(order="F")[mask.ravel(order="F")], axis=-1) * dx
        )
        source_p[ind, :] *= scale
    return source_p
...
def scale_stress_sources(source, c0, flags, dt, dx, N, s_source_pos_index):
    ...
    """
    source.sxx = scale_stress_source(
        source,
        c0,
        flags.source_sxx,
        flags.source_p0,
    ...
    source.syy = scale_stress_source(
        source,
        c0,
        flags.source_syy,
        flags.source_p0,
    ...
    source.szz = scale_stress_source(
        source,
        c0,
        flags.source_szz,
        flags.source_p0,
    ...
    source.sxy = scale_stress_source(
        source, c0, flags.source_sxy, True, source.sxy, dt, N, dx, s_source_pos_index
    )
    source.sxz = scale_stress_source(
        source, c0, flags.source_sxz, True, source.sxz, dt, N, dx, s_source_pos_index
    )
    source.syz = scale_stress_source(
        source, c0, flags.source_syz, True, source.syz, dt, N, dx, s_source_pos_index
    )


def scale_stress_source(
    source,
    c0,
    is_source_exists,
    is_p0_exists,
    ...
    if is_source_exists:
        if source.s_mode == "dirichlet" or is_p0_exists:
            ...
            if c0.size == 1:
                ...
                source_val[s_index, :] = source_val[s_index, :] * (
                    2 * dt * c0[s_source_pos_index[s_index]] / (N * dx)
                )
    return source_val
...
def scale_velocity_sources(
    flags, source, kgrid, c0, dt, dx, dy, dz, u_source_pos_index
):
    ...
    source.uy = scale_velocity_source(
        flags.source_uy, source.u_mode, source.uy, c0, dt, u_source_pos_index, dy
    )
    source.uz = scale_velocity_source(
        flags.source_uz, source.u_mode, source.uz, c0, dt, u_source_pos_index, dz
    )


def scale_velocity_source_x(
    is_source_ux,
    source_u_mode,
    source_val,
    kgrid,
    ...
    if is_nonuniform_grid:
        source_val = scale_velocity_source_nonuniform(
            is_source_ux, source_u_mode, kgrid, source_val, c0, dt, u_source_pos_index
        )
    else:
        source_val = scale_velocity_source(
            is_source_ux, source_u_mode, source_val, c0, dt, u_source_pos_index, dx
        )
    return source_val


def scale_velocity_source(
    is_source, source_u_mode, source_val, c0, dt, u_source_pos_index, d_direction
):
    """
    ...
    if c0.size == 1:
        ...
        u_index = range(source_val.shape[0])
        source_val[u_index, :] = source_val[u_index, :] * (
            2 * c0.ravel(order="F")[u_source_pos_index[u_index]] * dt / d_direction
        )
    return source_val


def scale_velocity_source_nonuniform(
    is_source, source_u_mode, kgrid, source_val, c0, dt, u_source_pos_index
):
    """
    if source.u_mode is not set to 'dirichlet', scale the d_direction
    velocity source terms by 2*dt*c0/dz to account for the time step and
    ...
    for u_index in range(source_val.size[0]):
        if c0.size == 1:
            # compute the scale parameter based on the homogeneous sound speed
            source_val[u_index, :] = source_val[u_index, :] * (
                2 * c0 * dt / (grid_point_sep[u_source_pos_index[u_index]])
            )
        else:
            # compute the scale parameter based on the sound speed at that position
            source_val[u_index, :] = source_val[u_index, :] * (
                2
                * c0[u_source_pos_index[u_index]]
                * dt
                / (grid_point_sep[u_source_pos_index[u_index]])
            )
    return source_val


def scale_transducer_source(
    is_transducer_source, transducer_input_signal, c0, dt, dx, u_source_pos_index
):
    """
    ...
    if is_transducer_source:
        if c0.size == 1:
            ...
            transducer_input_signal = transducer_input_signal * (
                2 * (np.mean(c0.flatten(order="F")[u_source_pos_index - 1])) * dt / dx
            )
    return transducer_input_signal
